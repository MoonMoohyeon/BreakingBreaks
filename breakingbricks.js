...
function cheat(){
  if(oncheat){
    oncheat=false;
    $("#cheat").text("현재 : Off");
    $("#cheat").css("backgroundColor","white");
    $("#cheat").css("color","black");
  }else{
    oncheat=true;
    $("#cheat").text("현재 : On");
    $("#cheat").css("backgroundColor","black");
...
function version1(){
  ...
  version = 1;
  blockset();
  $("#gamecontent").css({"backgroundImage" : 'url("image/DB.jpg")', "background-size" : "500px",  "background-repeat": "no-repeat",  "background-position": "center"});
...
function version2(){
  ...
  version = 2;
  blockset();
  $("#gamecontent").css({"backgroundImage" : 'url("image/BT.jpg")',"background-size" : "800px 500px",  "background-repeat": "no-repeat",  "background-position": "center"});
...
function version3(){
  ...
  version = 3;
  blockset();
  $("#gamecontent").css({"backgroundImage" : 'url("image/MZ.jpg")',"background-size" : "800px 500px",  "background-repeat": "no-repeat",  "background-position": "center"});
...
function adagio(){  // 공 느려지게 하는 아이템
  if(Math.abs(dx)>2 && Math.abs(dy)>2){ // 속도가 2 아래로는 떨어지지 않음
    if(dx<0){
      dx++;
    }else{
      dx--;
...
function allegro(){   // 속도가 빨라지는 아이템
  ...
  setTimeout(function (){   // 5초뒤 속도가 다시 느려짐
    adagio();
  },5000);
}
